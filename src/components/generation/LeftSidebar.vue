<template>
  <div
    class="relative flex flex-col h-full overflow-hidden bg-gray-900 border-r border-gray-600"
    ...
    <div class="flex items-start justify-between px-4 py-4">
      <div class="flex flex-col gap-y-1">
        <h2 class="text-xl font-medium text-gray-50">Generate</h2>
        <p class="text-xs text-gray-400">Text-to-image generation</p>
      </div>
      <div class="">
        <Cog class="w-5 h-5 text-gray-300 cursor-pointer hover:text-gray-100 transition-colors" @click="showPresetModal = true"/>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-2 px-2 overflow-y-auto">
      <!-- Preset Section -->
      <div v-for="section in sections" :key="section.id" class="flex flex-col gap-4 px-3 py-3 bg-gray-800 rounded">
        <div class="flex items-center gap-2 text-xs font-semibold tracking-wider text-gray-300">
          <component :is="section.icon" class="w-4 h-4" />
          <div class="grow">{{ section.label }}</div>
        </div>
