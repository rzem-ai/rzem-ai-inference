...
const setup = () => {
  ...
  const uniforms = {
    ...
    topWavePosition: {
      value: new Vector3(props.topWavePosition?.x ?? 10.0, props.topWavePosition?.y ?? 0.5, props.topWavePosition?.rotate ?? -0.4),
    },
    middleWavePosition: {
      value: new Vector3(props.middleWavePosition?.x ?? 5.0, props.middleWavePosition?.y ?? 0.0, props.middleWavePosition?.rotate ?? 0.2),
    },
    bottomWavePosition: {
      value: new Vector3(props.bottomWavePosition?.x ?? 2.0, props.bottomWavePosition?.y ?? -0.7, props.bottomWavePosition?.rotate ?? 0.4),
    },
...
<template>
  <div class="relative w-full h-full overflow-hidden bg-black floating-lines-panel rounded-xl">
    <!-- WebGL Canvas Background -->
    <div
      ref="containerRef"
      class="absolute inset-0 floating-lines-container"
      :style="{
...
<style scoped>
/* Canvas background layer - still interactive for mouse effects */
.floating-lines-container {
  z-index: 0;
}

/* Content layer sits above the canvas */
.floating-lines-panel > div:last-child {
  pointer-events: auto;
}
</style>
