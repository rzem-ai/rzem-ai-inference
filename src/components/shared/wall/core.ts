...
import type { LifecycleHook, VueRef, Watch } from './vue-lib-adapter';
...
export function useMasonryWall<T>({
  ...
  const resizeObserver = typeof ResizeObserver === 'undefined' ? undefined : new ResizeObserver(debounce(() => redraw()));
