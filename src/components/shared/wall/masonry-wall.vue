<template>
  <div ref="wall" class="masonry-wall" :style="{ display: 'flex', gap: `${gap}px` }">
    <div
      ...
      <div v-for="(itemIndex, row) in column" :key="keyMapper?.(items[itemIndex]!, columnIndex, row, itemIndex) ?? itemIndex" class="masonry-item">
        <slot :item="items[itemIndex]!" :column="columnIndex" :column-count="columns.length" :row="row" :index="itemIndex">
          {{ items[itemIndex] }}
        </slot>
      </div>
    </div>
...
import type { Column, NonEmptyArray } from './core';
import { useMasonryWall } from './core';
...
defineSlots<{
  default?: (props: { item: T; column: number; columnCount: number; row: number; index: number }) => VNode | VNode[] | Element | Element[];
}>();
