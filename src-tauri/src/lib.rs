...
async fn scan_and_discover_models(
    ...
    use crate::models::{scan_all_components, BundleBuilder, to_component_record};

    // Run scanner
    let components = scan_all_components()
        .map_err(|e| format!("Failed to scan components: {}", e))?;
    ...
    // Insert components
    let mut new_components = 0;
    for comp in &components {
        let record = to_component_record(comp);
        if db.insert_component(&record).is_ok() {
            new_components += 1;
        }
    }
    ...
    for bundle_def in bundles {
        ...
        if db.insert_bundle(&bundle_record).is_ok() {
    ...
    Ok(ScanResult {
        components_found: new_components,
...
pub fn run_with_config(runtime_config: shared::protocol::RuntimeConfig, port: Option<u16>) {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Scan for available models on startup (background task)
            tauri::async_runtime::spawn({
