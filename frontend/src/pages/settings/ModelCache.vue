  ...
  <div class="flex flex-col gap-4 p-4 overflow-y-auto">
      ...
      <div class="grid grid-cols-2 gap-3">
        <div class="bg-white rounded-xl border border-slate-200 p-4">
          ...
          <div v-if="settingsStore.diskUsage" class="text-xs text-slate-400 mt-1">
            {{ formatBytes(settingsStore.diskUsage.hf_cache_size) }} used
          </div>
        </div>
        <div class="bg-white rounded-xl border border-slate-200 p-4">
          ...
          <div v-if="settingsStore.diskUsage" class="text-xs text-slate-400 mt-1">
            {{ formatBytes(settingsStore.diskUsage.output_size) }} used
          </div>
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      <div v-else class="flex flex-col gap-2">
        <div
          v-for="model in settingsStore.cachedModels"
          :key="model.repo_id"
          ...
          <button
            ...
            @click="handleDelete(model.repo_id)">
            <Trash2 :size="14" />
          </button>
        </div>
