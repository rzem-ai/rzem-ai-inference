<template>
	<div class="flex h-full flex-col gap-2">
				...
				<div class="flex gap-2">
					...
					<Popover ref="sortPopover">
						<div class="flex min-w-40 flex-col gap-1 p-1">
							<div class="px-2 py-1 text-xs font-semibold tracking-wide text-slate-400 uppercase">Sort by</div>
							<button
								v-for="opt in styleSortOptions"
								:key="opt.value"
								class="flex w-full items-center justify-between gap-2 rounded-lg px-3 py-1 text-left transition-colors"
								:class="stylesStore.sortBy === opt.value ? 'bg-blue-50 text-blue-600' : 'text-slate-700 hover:bg-slate-50'"
							...
							<Divider class="my-1!" />
							<button
								class="flex w-full items-center gap-2 rounded-lg px-3 py-1 text-left text-slate-700 transition-colors hover:bg-slate-50"
		...
		<!-- Styles grid area -->
		<div ref="containerRef" class="min-h-0 flex-1 overflow-hidden rounded-xl">
			<!-- Empty state -->
			<div v-if="!stylesStore.loading && stylesStore.styles.length === 0" class="flex h-full items-center justify-center">
				<div class="text-center">
					<Palette :size="48" class="mx-auto mb-3 text-slate-300" />
					<p class="text-sm text-slate-400">No styles yet</p>
					<p class="mt-1 text-xs text-slate-300">Create a style to define reusable prompt templates</p>
					<RouterLink
						:to="{ name: 'styles-new' }"
						class="mt-4 inline-flex items-center gap-1 rounded-lg bg-blue-500 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-600">
						<Plus :size="16" />
			...
			<VirtualGrid v-else :items="rows" :item-size="rowHeight">
